...
class WorkUpdate extends StatefulWidget {
  final int id;
  WorkUpdate({Key key, @required this.id}) : super(key: key);
  @override
  _WorkUpdateState createState() => _WorkUpdateState(id);
}

class _WorkUpdateState extends State<WorkUpdate> {
  ...
  Future<String> getUser() async {
    ...
    try {
      http.Response item = await http.get(
          Uri.encodeFull(
              "https://kariernesia.com/jwt/profile/exp/" + id.toString()),
  ...
  Future<List> _save() async {
    ...
    final response = await http.post(
        "https://kariernesia.com/jwt/profile/exp/update?token=" + _token,
        body: data);
    ...
    try {
      ...
      if (result.isNotEmpty && result[0].rawAddress.isNotEmpty) {
        ...
        new Future.delayed(Duration(seconds: 2), () {
          setState(() {
            isLoading = false;
            _jobController.text = dataUser["job_title"];
            _reportController.text = dataUser["report_to"];
            _startcontroller.text = dataUser["start_date"];
            _tillcontroller.text = dataUser["end_date"];
            _companyController.text = dataUser["company"];
            _jobDescController.text = dataUser["job_desc"];
            _joblevel = dataUser["joblevel_id"];
            _jobtype = dataUser["jobtype_id"];
            _fungsikerja = dataUser["fungsikerja_id"];
            _industri = dataUser["industri_id"];
            _city = dataUser["city_id"];
            _salary = dataUser["salary_id"];
          });
        });
      }
